...
def linprog(c, A_ub=None, b_ub=None, A_eq=None, b_eq=None,
    ...
    ----------
    c : 1D array
        The coefficients of the linear objective function to be minimized.  目标函数中的系数
    A_ub : 2D array, optional
        The inequality constraint matrix. Each row of ``A_ub`` specifies the
        coefficients of a linear inequality constraint on ``x``.                     不等式的约束矩阵
    b_ub : 1D array, optional
        The inequality constraint vector. Each element represents an                不等式右侧的值
        upper bound on the corresponding value of ``A_ub @ x``.
    A_eq : 2D array, optional
        The equality constraint matrix. Each row of ``A_eq`` specifies the          等式的约束矩阵
        coefficients of a linear equality constraint on ``x``.
    b_eq : 1D array, optional                                                                 
        The equality constraint vector. Each element of ``A_eq @ x`` must equal    等式右侧的值
        the corresponding element of ``b_eq``.
    bounds : sequence, optional
        A sequence of ``(min, max)`` pairs for each element in ``x``, defining         决策变量的范围
    ...
    callback : callable, optional
        ...
        `scipy.optimize.OptimizeResult` consisting of the following fields:       结果

            x : 1D array
                The current solution vector.                     当前的解向量，每个决策变量的值
            fun : float
                The current value of the objective function ``c @ x``.   目标函数的当前值
            success : bool
                ``True`` when the algorithm has completed successfully.    标志位，表示函数运行成功与否
            slack : 1D array
                The (nominally positive) values of the slack（松弛）,       松弛变量的值       
                ``b_ub - A_ub @ x``.
            con : 1D array
                The (nominally zero) residuals of the equality constraints, 残差，剩余变量的值
                ``b_eq - A_eq @ x``.
            phase : int
                The phase of the algorithm being executed.                    正在执行算法的阶段
            status : int
